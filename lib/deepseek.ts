...
你的 JSON 必须额外包含 "assessment" 字段，对该融合方案做出客观的结构化评估：
- overall_grade: 综合评级（S/A+/A/B+/B/C/D 之一）
- tech_match: 技术匹配度评级
- license_risk: 许可证风险评级
- coverage: 功能覆盖度评级
- maintenance_risk: 维护风险评级
- warnings: 需要关注的警告事项列表（每条一句话，面向非技术用户，用通俗中文）
- strengths: 方案优点列表（每条一句话，用通俗中文）
- summary: 一句话总结，用通俗中文告诉用户这个方案靠不靠谱、需要注意什么
...
对于分析出的每个重大缺口：
1. github_search_term 必须是具体的 GitHub 仓库全名（如 "alipay/alipay-easysdk"、"stripe/stripe-node"、"plataformatec/devise"），而不是宽泛的关键词。如果你不能确定一个真实存在的仓库，把 github_search_term 设为空字符串 ""。
2. description 控制在 30 字以内，用通俗中文说明缺了这个功能对项目有什么影响。
3. 只列出真正缺失的核心功能，不要为了凑数而编造缺口。如果没有明显缺失，gaps 为空数组 []。
...
你必须仅输出一个绝对合法、无多余转义符的纯 JSON 对象，其 Schema 规范完全对标如下结构：
{
  ...
  "assessment": {
    ...
    "coverage": "B+",
    "maintenance_risk": "B",
    "warnings": ["shadcn-ui/ui 超过 1 年未更新，建议找替代品"],
    "strengths": ["各仓库功能互补性强", "许可证兼容性好"],
    "summary": "方案总体可行，建议关注 shadcn-ui 的维护状态，考虑寻找替代方案。"
  },
